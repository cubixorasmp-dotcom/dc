...
function countdownText(endsAt) {
  const timestamp = Math.floor(endsAt / 1e3);
  return `Bitiş: <t:${timestamp}:F>\nCanlı geri sayım: <t:${timestamp}:R>\nKalan: **${formatRemaining(endsAt - Date.now())}**`;
}
...
var commands = [
  ...
  new SlashCommandBuilder().setName("kick").setDescription("\xDCyeyi sunucudan atar").addUserOption((option) => option.setName("uye").setDescription("At\u0131lacak uye").setRequired(true)).addStringOption((option) => option.setName("sebep").setDescription("Atma sebebi")),
  ...
  new SlashCommandBuilder().setName("owner-list").setDescription("Bot owner listesini g\xF6sterir").setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  new SlashCommandBuilder().setName("owner").setDescription("Yönetici owner komutlarını gösterir").setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
...
var countdownTimers = /* @__PURE__ */ new Map();
async function getStoredMessage(record) {
...
function startCountdown(id, update) {
  stopCountdown(id);
  const timer = setInterval(() => void update(), 30e3);
...
async function finishGiveaway(id) {
  ...
  if (!record) return;
  const { channel, message } = await getStoredMessage(record);
  ...
  const finishedEmbed = new EmbedBuilder().setTitle(`\u{1F389} ${record.title ?? "Çekiliş"}`).setDescription(`Ödül: **${record.firstPrize}**${record.secondPrize ? `
İkinci ödül: **${record.secondPrize}**` : ""}

**Çekiliş sona erdi.**`).addFields({ name: "Kazananlar", value: mention }).setColor(5763719).setTimestamp();
  await message?.edit({ content: `\u{1F389} **Çekiliş bitti!**\nKazananlar: ${mention}`, embeds: [finishedEmbed] }).catch(() => void 0);
  if (channel?.isTextBased() && "send" in channel) {
    await channel.send(`\u{1F389} Tebrikler ${mention}!`).catch(() => void 0);
  }
  store.removeGiveaway(id);
}
async function updateGiveawayMessage(id) {
  const record = store.giveaways().find((giveaway) => giveaway.id === id);
  if (!record || record.endsAt <= Date.now()) return;
  const { message } = await getStoredMessage(record);
  if (!message) return;
  const embed = new EmbedBuilder().setTitle(`\u{1F389} ${record.title ?? "Çekiliş"}`).setDescription(`Ödül: **${record.firstPrize}**${record.secondPrize ? `
İkinci ödül: **${record.secondPrize}**` : ""}

Katılmak için \u{1F389} tepkisine basın.`).addFields({ name: "Geri sayım", value: countdownText(record.endsAt) }, { name: "Kazanan sayısı", value: `${record.winnerCount}`, inline: true }).setColor(16762967).setFooter({ text: "Katılım açık • Süre 30 saniyede bir yenilenir" }).setTimestamp(record.endsAt);
  await message.edit({ embeds: [embed] }).catch(() => void 0);
}
function scheduleGiveaway(id, delay) {
  startCountdown(id, () => updateGiveawayMessage(id));
  if (delay <= 2147e6) {
...
async function closePoll(id) {
  stopCountdown(id);
  const record = store.polls().find((poll) => poll.id === id);
  if (!record) return;
  const { message } = await getStoredMessage(record);
  const closedEmbed = new EmbedBuilder().setTitle(`\u{1F4CA} ${record.title}`).setDescription(record.choices.map((choice, index) => `${["1\uFE0F\u20E3", "2\uFE0F\u20E3", "3\uFE0F\u20E3", "4\uFE0F\u20E3"][index]} ${choice}`).join("\n")).addFields({ name: "Durum", value: "Anket kapandı; oylar artık değiştirilebilir olsa da süre doldu." }).setColor(9807270).setFooter({ text: "Anket sona erdi" }).setTimestamp();
  await message?.edit({ content: "\u23F0 **Anket süresi doldu.**", embeds: [closedEmbed] }).catch(() => void 0);
  store.removePoll(id);
}
async function updatePollMessage(id) {
  const record = store.polls().find((poll) => poll.id === id);
  if (!record || !record.endsAt || record.endsAt <= Date.now()) return;
  const { message } = await getStoredMessage(record);
  if (!message) return;
  const embed = new EmbedBuilder().setTitle(`\u{1F4CA} ${record.title}`).setDescription(record.choices.map((choice, index) => `${["1\uFE0F\u20E3", "2\uFE0F\u20E3", "3\uFE0F\u20E3", "4\uFE0F\u20E3"][index]} ${choice}`).join("\n")).addFields({ name: "Geri sayım", value: countdownText(record.endsAt) }).setColor(5763719).setFooter({ text: "Oy vermek için seçenek emojisine basın" }).setTimestamp(record.endsAt);
  await message.edit({ embeds: [embed] }).catch(() => void 0);
}
function schedulePoll(id, delay) {
  startCountdown(id, () => updatePollMessage(id));
  if (delay <= 2147e6) {
    setTimeout(() => void closePoll(id), Math.max(0, delay));
    return;
  }
  setTimeout(() => schedulePoll(id, delay - 2147e6), 2147e6);
}
async function handleCommand(interaction) {
  ...
  if (command === "yardim") {
    await interaction.reply("Komutlar: `/ip`, `/aktif`, `/site`, `/profil`, `/sil`, `/ban`, `/kick`, `/mute`, `/owner`, `/cekilis`, `/anket`, `/ticket-kur`, `/kelime-kanal`, `/sayisayma`, `/koruma-rol`, `/likkoruma` ve `/bakim`. Prefix: `!ban`, `!kick`, `!mute`, `!owner`.");
    return;
  }
  if (command === "ip") {
  ...
  if (command === "owner" || command === "owner-list") {
    if (!canManage(member, PermissionFlagsBits.Administrator)) {
      await interaction.reply({ content: "Bu komut için Yönetici izni gerekli.", ephemeral: true });
      return;
    }
    const owners = store.owners();
    const list = owners.length ? owners.map((id) => `<@${id}>`).join(", ") : "Hen\xFCz eklenmi\u015F owner yok.";
    await interaction.reply({ content: `**Owner komutları**\n/owner-ekle @üye\n/owner-cikar @üye\n/owner-list\n\n**Owner listesi:** ${list}`, ephemeral: true });
    return;
  }
  ...
  if (command === "kick") {
    if (!canManage(member, PermissionFlagsBits.KickMembers)) return void interaction.reply({ content: "Üyeleri At yetkisi gerekli.", ephemeral: true });
    const user = interaction.options.getUser("uye", true);
    ...
    await logDiscord(interaction.guild.id, `${user.tag} ${interaction.user} tarafından atıldı. Sebep: ${reason}`);
    return;
  }
  if (command === "ban") {
  ...
  if (command === "anket") {
    ...
    if (amount > 0 && !duration) return void interaction.reply({ content: "Geçersiz anket süresi.", ephemeral: true });
    const end = duration ? Date.now() + duration : null;
    const embed = new EmbedBuilder().setTitle(`\u{1F4CA} ${title}`).setDescription(choices.map((choice, index) => `${["1\uFE0F\u20E3", "2\uFE0F\u20E3", "3\uFE0F\u20E3", "4\uFE0F\u20E3"][index]} ${choice}`).join("\n")).addFields({ name: "Katılım", value: "Oy vermek için seçenek emojisine basın." }, { name: "Süre", value: end ? countdownText(end) : "Süresiz anket" }).setColor(5763719).setFooter({ text: "Cubixora Anket • Sonuçlar süre bitince kapanır" });
    ...
    if (end) {
      ...
      store.addPoll({ id, guildId: interaction.guild.id, channelId: message.channel.id, messageId: message.id, title, choices, endsAt: end });
      schedulePoll(id, duration);
    }
    return;
  }
  if (command === "cekilis") {
    ...
    const embed = new EmbedBuilder().setTitle(`\u{1F389} ${title}`).setDescription(`Ödül: **${firstPrize}**${secondPrize ? `
İkinci ödül: **${secondPrize}**` : ""}

Katılmak için \u{1F389} tepkisine basın.`).addFields({ name: "Geri sayım", value: countdownText(end) }, { name: "Kazanan sayısı", value: `${winnerCount}`, inline: true }).setColor(16762967).setFooter({ text: "Çekiliş açık • Süre 30 saniyede bir yenilenir" }).setTimestamp(end);
    const message = await interaction.reply({ embeds: [embed], fetchReply: true });
    await message.react("\u{1F389}");
    const id = `${interaction.guild.id}-${message.id}`;
    store.addGiveaway({ id, guildId: interaction.guild.id, channelId: message.channel.id, messageId: message.id, title, firstPrize, secondPrize, winnerCount, endsAt: end });
...
async function handlePrefix(message) {
  ...
  if (command === "ip") await message.reply({ embeds: [await createIpEmbed()] });
    ...
    if (!canManage(authorMember, PermissionFlagsBits.Administrator)) {
      await message.reply("Bu komut için Yönetici izni gerekli.");
      return;
    }
    const owners = store.owners();
    await message.reply(`**Owner komutları**\n!owner-ekle @üye\n!owner-cikar @üye\n!owner\n\n**Owner listesi:** ${owners.length ? owners.map((id) => `<@${id}>`).join(", ") : "Owner yok."}`);
  }
    ...
    const authorMember = await message.guild.members.fetch(message.author.id).catch(() => null);
    if (!canManage(authorMember, PermissionFlagsBits.BanMembers)) {
      await message.reply("Bu komut için Üyeleri Yasakla yetkisi gerekli.");
      return;
    }
    const target = message.mentions.users.first();
    if (!target) {
      await message.reply("Kullanım: `!ban @üye sebep`");
      return;
    }
    await message.guild.members.ban(target, { reason: trimReason(args.slice(1).join(" ")) });
    await message.reply(`${target.tag} yasaklandı.`);
  } else if (command === "kick") {
    const authorMember = await message.guild.members.fetch(message.author.id).catch(() => null);
    if (!canManage(authorMember, PermissionFlagsBits.KickMembers)) {
      await message.reply("Bu komut için Üyeleri At yetkisi gerekli.");
      return;
    }
    const target = message.mentions.users.first();
    if (!target) {
      await message.reply("Kullanım: `!kick @üye sebep`");
      return;
    }
    await message.guild.members.kick(target, trimReason(args.slice(1).join(" ")));
    await message.reply(`${target.tag} sunucudan atıldı.`);
  } else if (command === "mute") {
    const authorMember = await message.guild.members.fetch(message.author.id).catch(() => null);
    if (!canManage(authorMember, PermissionFlagsBits.ModerateMembers)) {
      await message.reply("Bu komut için Üyelere Zaman Aşımı Uygula yetkisi gerekli.");
      return;
    }
    const target = message.mentions.users.first();
    const amount = Number(args[1]);
    const unit = args[2] ?? "dakika";
    const duration = parseDuration(amount, unit);
    if (!target || !duration) {
      await message.reply("Kullanım: `!mute @üye 10 dakika sebep`");
      return;
    }
    const targetMember = await message.guild.members.fetch(target.id);
    await applyMute(targetMember, duration, trimReason(args.slice(3).join(" ")));
    await message.reply(`${target.tag} ${amount} ${unit} susturuldu.`);
  }
}
...
client.once(Events.ClientReady, async (readyClient) => {
  ...
  for (const poll of store.polls()) schedulePoll(poll.id, poll.endsAt - Date.now());
